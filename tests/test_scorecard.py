...
def test_build_scorecard_excludes_legislation_task_from_overall():
    ...
    scorecard = build_scorecard(
        ...
        legislation_task_ids={"legislation_lex_au"}, legislation_service_ids=set(),
    )
...
def test_build_scorecard_excludes_legislation_chat_service_from_overall():
    chat_results = [
        _chat_result("passport", 8.0),
        _chat_result("file_tax", 7.0),
        _chat_result("legislation_lex_au", 9.0),  # must NOT count toward overall
    ]
    agent_results = [_agent_result("passport_agent", 6.0), _agent_result("ato_agent", 4.0)]

    scorecard = build_scorecard(
        chat_results, agent_results,
        legislation_task_ids=set(), legislation_service_ids={"legislation_lex_au"},
    )

    assert scorecard.chat_mean == 7.5  # mean of 8.0 and 7.0 only
    assert scorecard.agent_mean == 5.0
    assert scorecard.overall_score == 6.25
...
def test_build_legislation_scorecard_raises_on_missing_agent_result():
    chat = [_chat_result("legislation_lex_au", 9.0)]
    agent = [_agent_result("legislation_austlii", 5.0)]  # no result for legislation_lex_au

    with pytest.raises(ValueError, match="legislation_lex_au"):
        build_legislation_scorecard(chat, agent)
